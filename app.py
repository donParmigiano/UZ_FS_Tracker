...
from src.dashboard.pages import PAGE_SPECS
...
def render_tabs(context: dict) -> None:
    """Render tabs from the shared page registry."""
    tabs = st.tabs([spec.title for spec in PAGE_SPECS])
    for tab, spec in zip(tabs, PAGE_SPECS):
        with tab:
            spec.render(context)
