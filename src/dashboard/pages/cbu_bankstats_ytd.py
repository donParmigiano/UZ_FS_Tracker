...
def render() -> None:
    """Render YTD summary metrics, inventory/QA tables, and master preview filters."""
    ...
    if summary_df.empty and inventory_df.empty and qa_df.empty and master_df.empty:
        st.info("YTD outputs are not available yet. Run `python collect_cbu_bankstats_ytd.py` first.")
        return
    ...
    if not summary_df.empty:
        ...
        months_processed = int((monthly["excel_files_found"] > 0).sum()) if "excel_files_found" in monthly else len(monthly)
        reports_found = int(total["reports_found"].iloc[0]) if not total.empty else int(monthly.get("reports_found", pd.Series(dtype=int)).sum())
        files_downloaded = int(total["excel_files_downloaded"].iloc[0]) if not total.empty else int(monthly.get("excel_files_downloaded", pd.Series(dtype=int)).sum())
        parsed_csv_files = int(total["parsed_csv_files"].iloc[0]) if not total.empty else int(monthly.get("parsed_csv_files", pd.Series(dtype=int)).sum())
        master_row_count = int(total["master_row_count"].iloc[0]) if (not total.empty and "master_row_count" in total.columns) else len(master_df)
    ...
    if not master_df.empty:
        ...
        month_options = ["All"] + sorted(master_df["month"].dropna().astype(str).unique().tolist()) if "month" in master_df else ["All"]
        title_options = ["All"] + sorted(master_df["report_title"].dropna().astype(str).unique().tolist()) if "report_title" in master_df else ["All"]
        sheet_options = ["All"] + sorted(master_df["sheet_name"].dropna().astype(str).unique().tolist()) if "sheet_name" in master_df else ["All"]

        f1, f2, f3 = st.columns(3)
        selected_month = f1.selectbox("Filter month", options=month_options, key="ytd_master_month")
        selected_title = f2.selectbox("Filter report title", options=title_options, key="ytd_master_title")
        selected_sheet = f3.selectbox("Filter sheet name", options=sheet_options, key="ytd_master_sheet")
