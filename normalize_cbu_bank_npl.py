"""Parse recurring CBU report: Information on Non-performing loans of commercial banks.

Run:
    python normalize_cbu_bank_npl.py
    python normalize_cbu_bank_npl.py --overwrite
    python normalize_cbu_bank_npl.py --periods 2026_04 --overwrite
"""
...
MASTER_OUTPUT = Path("data/master/cbu_bank_npl_master.csv")
QA_OUTPUT = Path("data/master/cbu_bank_npl_parse_qa.csv")
FILE_NAME_CONTAINS = "information-on-non_performing-loans-of-commercial-banks"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Parse CBU NPL Excel files into master + QA CSV.")
...
def parse_workbook(file_path: Path, loaded_at: str) -> tuple[list[MasterRow], QAEntry]:
    ...
    unit_monetary = clean_text(resolve_cell_value(sheet, 2, 5)) or "billion UZS"

    indicator_map = {
        3: ("total_loan_portfolio", unit_monetary),
        4: ("npl", unit_monetary),
        5: ("npl_share", "percent"),
    }
    ...
    for row_idx in range(1, sheet.max_row + 1):
        ...
        is_total = "banking system" in lower_a
        is_state_group = "banks with state ownership" in lower_a
        is_other_group = "other banks" in lower_a
        ...
        numeric_cells: list[tuple[str, float, str, str]] = []
        for col_idx, (indicator, unit) in indicator_map.items():
            numeric_value = parse_number(resolve_cell_value(sheet, row_idx, col_idx))
            if numeric_value is None:
                continue
            cell_ref = f"{get_column_letter(col_idx)}{row_idx}"
            numeric_cells.append((indicator, numeric_value, unit, cell_ref))
        ...
        rows_read += 1
        for indicator, numeric_value, unit, cell_ref in numeric_cells:
            rows.append(
                MasterRow(
    ...
    notes = "Parsed successfully." if rows else "No numeric rows in C:E for recognized entities."
...
def main() -> int:
    ...
    if not files:
        print("No matching files found for Information-on-Non_performing-loans-of-commercial-banks.")
        return 1
