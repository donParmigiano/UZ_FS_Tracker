...
REPORT_LINK_PATTERN = re.compile(
    r'href=["\']([^"\']+/statistics/bankstats/[^"\']+)["\']', re.IGNORECASE
)
FILE_LINK_PATTERN = re.compile(
    r'href=["\']([^"\']+\.(?:xlsx|xls))(?:\?[^"\']*)?["\']', re.IGNORECASE
)
...
def fetch_text(url: str, timeout: int = 45) -> str:
    req = Request(
        url, headers={"User-Agent": "Mozilla/5.0 (compatible; CBU-Collector-YTD/1.0)"}
    )
...
def extract_report_links(listing_html: str) -> list[str]:
    links = [
        ...
        for match in REPORT_LINK_PATTERN.finditer(listing_html)
    ]
...
def extract_excel_links(report_html: str, report_url: str) -> list[str]:
    links = [
        urljoin(report_url, match.group(1))
        for match in FILE_LINK_PATTERN.finditer(report_html)
    ]
    return unique_ordered(links)
...
def safe_sheet_slug(sheet_name: str) -> str:
    return (
        re.sub(r"[^A-Za-z0-9._-]+", "_", (sheet_name or "sheet").strip())[:80]
        or "sheet"
    )
...
def export_parse_summary(records: list[SourceRecord], master_row_count: int) -> None:
        ...
        rows.append(
            {
                "month": month,
                "reports_found": len(
                    {r.report_url for r in month_records if r.report_url}
                ),
                "excel_files_found": len(month_records),
                "excel_files_downloaded": sum(
                    r.download_status
                    in {"downloaded", "skipped_existing", "parse_only"}
                    for r in month_records
                ),
                "files_parsed": sum(
                    r.parse_status in {"parsed", "skipped_existing"}
                    for r in month_records
                ),
    ...
    summary_df.loc[summary_df["month"] == "TOTAL", "master_row_count"] = (
        master_row_count
    )
...
def export_parse_qa(qa_rows: list[QARow]) -> None:
    ...
    pd.DataFrame([asdict(row) for row in qa_rows], columns=columns).to_csv(
        PARSE_QA_CSV, index=False
    )
...
def collect_month(
    month: str, parse_only: bool, overwrite: bool
) -> tuple[list[SourceRecord], list[QARow], list[dict[str, object]], int]:
        ...
        for local_file in raw_files:
            ...
            try:
                (
                    ...
                    master_count,
                    skipped,
                ) = parse_excel_to_csv(local_file, processed_dir, month, "", overwrite)
                rec.rows_total = rows_total
                rec.sheets_total = sheets_total
                rec.csv_files_total = csv_total
                rec.parse_status = (
                    "skipped_existing" if skipped and not overwrite else "parsed"
                )
                qa_rows.extend(qa)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect 2026 YTD CBU banking statistics"
    )
    parser.add_argument(
        "--parse-only", action="store_true", help="Only parse existing raw Excel files"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing raw and parsed outputs",
    )
...
def run(parse_only: bool, overwrite: bool) -> None:
    ...
    for month in MONTHS:
        records, qa_rows, master_rows, master_count = collect_month(
            month, parse_only=parse_only, overwrite=overwrite
        )
